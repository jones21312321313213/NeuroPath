...
function RecordProgressForm({
  ...
  const isCustomInitial = defaultDomain && !availableDomains.includes(defaultDomain);

  const [selectedSubjectOption, setSelectedSubjectOption] = useState(
    isCustomInitial ? "__custom__" : defaultDomain,
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMessage(
        "Failed to record progress. Please check your connection and try again.",
      );
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="rpm-modal-body">
      ...
      {selectedSubjectOption === "__custom__" && (
        <div className="form-group rpm-form-group">
          <label
            htmlFor="rpm-custom-subject-input"
          ...
          <input
            ...
            value={
              isNaN(parseInt(score, 10))
                ? 0
                : Math.max(0, Math.min(100, parseInt(score, 10)))
            }
  ...
  return (
    ...
    >
      <div className="rpm-modal-container" ref={modalRef}>
        ...
        {/* Body Form with unique key per student & initialSubject */}
        <RecordProgressForm
          key={`${student.studentID}-${initialSubject || "default"}`}
          student={student}
