...
class IEPDashboardAndInsightEndpointsTests(TestCase):
    """Test dashboard stats and student insight endpoints."""

    def setUp(self):
        self.client = APIClient()
        self.user1, self.teacher1, self.token1 = create_teacher_with_login('dash1@example.com')
        self.user2, self.teacher2, self.token2 = create_teacher_with_login('dash2@example.com')
        self.student1 = create_student(self.teacher1, name='Dash Student')
        self.iep1 = IEPModel.objects.create(studentID=self.student1, version=1, goals='Goals')
        self.insight1 = GeneratedAIInsight.objects.create(
            student=self.student1,
            teacher=self.user1,
            summary_text='Student is progressing well.',
        )
    ...
    def test_dashboard_stats_returns_teacher_scoped_counts(self):
        self._auth(self.token1)
        response = self.client.get('/api/iep/dashboard-stats/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get('active_ieps'), 1)
        self.assertEqual(response.data.get('ai_insights'), 1)

    def test_dashboard_stats_isolated_from_other_teacher(self):
        self._auth(self.token2)
        response = self.client.get('/api/iep/dashboard-stats/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get('active_ieps'), 0)
        self.assertEqual(response.data.get('ai_insights'), 0)

    def test_get_student_insights_requires_auth(self):
        response = self.client.get(f'/api/iep/student/{self.student1.pk}/insights/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_get_student_insights_returns_insights_for_owner(self):
        self._auth(self.token1)
        response = self.client.get(f'/api/iep/student/{self.student1.pk}/insights/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
        self.assertEqual(response.data[0]['summary_text'], 'Student is progressing well.')
